    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-800 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-amber-600 to-amber-400 flex items-center justify-center text-white shadow-lg shadow-amber-600/30">
            <AlertCircle className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white tracking-wide">Forget Punch Regularization</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-card p-4 rounded-xl border border-slate-800 bg-slate-900/60 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-medium">Total Regularizations</p>
        ...
        <div className="glass-card p-4 rounded-xl border border-slate-800 bg-slate-900/60 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-medium">Regularized & Approved</p>
            <h3 className="text-2xl font-bold text-emerald-400 mt-1 font-mono">{stats.approved}</h3>
          </div>
        ...
        <div className="glass-card p-4 rounded-xl border border-slate-800 bg-slate-900/60 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-medium">Pending Dept Approvals</p>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="glass-card p-5 border border-slate-800 rounded-2xl bg-slate-900/70 backdrop-blur-md shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
              ...
              <div>
                <label className="block text-slate-300 font-medium mb-1">Date of Missed Punch</label>
                <input
                  type="date"
                  value={date}
              ...
              {/* Missing Punch Type */}
              <div>
                <label className="block text-slate-300 font-medium mb-1">Missing Punch Type</label>
              ...
              {/* Actual Time */}
              <div>
                <label className="block text-slate-300 font-medium mb-1">Actual Punch Time (Estimated)</label>
                <input
                  type="text"
                  value={actualTime}
                  onChange={(e) => setActualTime(e.target.value)}
                  placeholder="e.g. 08:30 or 17:15"
                  className="w-full bg-slate-950/80 border border-slate-700 rounded-xl px-3 py-2 text-white font-mono focus:border-indigo-500 focus:outline-none"
                  required
                />
              </div>
              ...
              {/* Reason */}
              <div>
                <label className="block text-slate-300 font-medium mb-1">Reason for Missing Punch</label>
                <textarea
                  rows={3}
                  value={reason}
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-semibold text-xs shadow-md shadow-amber-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-60"
                >
          ...
          <div className="glass-card p-5 border border-slate-800 rounded-2xl bg-slate-900/70 backdrop-blur-md shadow-xl space-y-4 flex flex-col h-full">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-white">Forget Punch History & Department Verification</h2>
                <span className="text-[11px] px-2 py-0.5 rounded-full bg-amber-900/60 text-amber-300 font-mono border border-amber-700/40">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full bg-slate-950/80 border border-slate-700 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none"
                />
              </div>
            </div>
                  ...
                  {filteredRecords.length === 0 ? (
                    ...
                    filteredRecords.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-800/30">
                        <td className="py-2.5 px-3 text-amber-400 font-semibold">{r.id}</td>
                        <td className="py-2.5 px-3 font-sans">
                          <div className="font-medium text-slate-200">{r.empName}</div>
                        ...
                        <td className="py-2.5 px-3 font-sans">
                          ...
                          <span className="inline-block mt-0.5 px-1.5 py-0.5 rounded bg-slate-800 text-amber-300 font-mono text-[10px]">
                            {r.punchType} Punch
                          </span>
                        </td>
                        <td className="py-2.5 px-3 text-slate-200 font-bold">{r.actualTime}</td>
                          ...
                          {r.status === 'Pending' ? (
                            <div className="flex items-center justify-center gap-1">
                              <button
                                ...
                                className="p-1 rounded-md bg-emerald-950 text-emerald-400 hover:bg-emerald-900 border border-emerald-800/60"
                              >
                              ...
                              <button
                                ...
                                className="p-1 rounded-md bg-rose-950 text-rose-400 hover:bg-rose-900 border border-rose-800/60"
                              >
