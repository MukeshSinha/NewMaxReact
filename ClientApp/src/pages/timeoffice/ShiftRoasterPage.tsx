...
export const ShiftRoasterPage: React.FC = () => {
  ...
  const getShiftBadgeClass = (s: string) => {
    switch (s) {
      case 'WO':
        return 'bg-blue-950/80 text-blue-400 border border-blue-800/60 font-bold';
      case 'A':
        return 'bg-emerald-950/80 text-emerald-400 border border-emerald-800/60 font-bold';
      case 'B':
        return 'bg-amber-950/80 text-amber-400 border border-amber-800/60 font-bold';
      case 'C':
        return 'bg-purple-950/80 text-purple-400 border border-purple-800/60 font-bold';
      case 'G1':
      case 'G2':
    ...
    <div className="space-y-6 max-w-full mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-800 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-white shadow-lg shadow-violet-600/30">
            <CalendarRange className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white tracking-wide">Monthly Shift Roaster</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium text-xs border border-slate-700 transition-all flex items-center gap-2"
          >
          ...
          <button
            ...
            disabled={saving}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold text-xs shadow-md shadow-emerald-600/30 transition-all flex items-center gap-2 disabled:opacity-60"
          >
      ...
      <div className="glass-card p-4 rounded-2xl border border-slate-800 bg-slate-900/70 backdrop-blur-md shadow-xl flex flex-wrap items-center justify-between gap-4 text-xs">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white font-medium focus:border-indigo-500"
            >
              {[
                { num: 1, name: 'January' },
                { num: 2, name: 'February' },
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-slate-950 border border-slate-700 rounded-lg pl-8 pr-3 py-1.5 text-white placeholder-slate-500 focus:border-indigo-500"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-indigo-950/80 hover:bg-indigo-900/80 text-indigo-300 border border-indigo-800/60 font-medium flex items-center gap-1.5"
          >
          ...
          <div className="hidden xl:flex items-center gap-2 border-l border-slate-800 pl-3">
            ...
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-emerald-950 text-emerald-400 border border-emerald-800/50">
              A (Morn)
            </span>
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-amber-950 text-amber-400 border border-amber-800/50">
              B (Eve)
            </span>
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-purple-950 text-purple-400 border border-purple-800/50">
              C (Night)
            </span>
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-slate-800 text-slate-300 border border-slate-700">
              G1 (Gen)
            </span>
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-blue-950 text-blue-400 border border-blue-800/50">
              WO (Off)
            </span>
          </div>
        </div>
      ...
      {/* Roster Grid Matrix */}
      <div className="glass-card rounded-2xl border border-slate-800 bg-slate-900/70 backdrop-blur-md shadow-xl overflow-hidden">
        <div className="overflow-x-auto max-h-[580px] custom-scrollbar">
          <table className="w-full text-[11px] text-left border-collapse">
            <thead>
              <tr className="bg-slate-950 border-b border-slate-800 text-slate-400 sticky top-0 z-20">
                <th className="py-2.5 px-3 font-semibold sticky left-0 bg-slate-950 z-30 min-w-[140px] border-r border-slate-800">
                  Emp Code & Name
                </th>
                <th className="py-2.5 px-2 font-semibold min-w-[80px] border-r border-slate-800">
                  Dept
                </th>
                {daysArray.map((day) => {
                  ...
                  return (
                    <th
                      key={day}
                      className={`py-2 px-1 text-center font-mono min-w-[34px] border-r border-slate-800/60 ${
                        isSunday ? 'bg-indigo-950/40 text-indigo-400 font-bold' : ''
                      }`}
                    >
                      <div>{day}</div>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {filteredRows.length === 0 ? (
                <tr>
                  <td colSpan={daysInMonth + 2} className="text-center py-10 text-slate-500 font-sans">
                    No roster entries found for the selected month and department.
                  </td>
                </tr>
              ) : (
                filteredRows.map((row) => (
                  <tr key={row.empCode} className="hover:bg-slate-800/30">
                    {/* Fixed Left Employee Column */}
                    <td className="py-2 px-3 sticky left-0 bg-slate-900/95 z-10 border-r border-slate-800">
                      <div className="font-semibold text-slate-200 font-sans truncate max-w-[130px]">
                        {row.empName}
                      </div>
                      <div className="text-[10px] text-indigo-400 font-mono">{row.empCode}</div>
                    </td>

                    {/* Department */}
                    <td className="py-2 px-2 text-slate-400 font-sans text-[10px] border-r border-slate-800 truncate max-w-[80px]">
                      {row.department}
                    </td>
                    ...
                    {daysArray.map((day) => {
                      ...
                      return (
                        <td
                          key={day}
                          className={`p-0.5 text-center border-r border-slate-800/40 ${
                            isSunday ? 'bg-indigo-950/20' : ''
                          }`}
                        >
                          <button
                            type="button"
