    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-800 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white tracking-wide">Weekoff Transfer</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-card p-4 rounded-xl border border-slate-800 bg-slate-900/60 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-medium">Total Transfer Requests</p>
        ...
        <div className="glass-card p-4 rounded-xl border border-slate-800 bg-slate-900/60 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 font-medium">Approved Transfers</p>
          ...
          <div className="glass-card p-5 border border-slate-800 rounded-2xl bg-slate-900/70 backdrop-blur-md shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="block text-slate-300 font-medium mb-1">Current Weekoff</label>
                  <select
                ...
                <div>
                  <label className="block text-slate-300 font-medium mb-1">Requested Weekoff</label>
                  <select
                    value={requestedWeekoff}
                    onChange={(e) => setRequestedWeekoff(e.target.value)}
                    className="w-full bg-slate-950/80 border border-indigo-700/60 rounded-xl px-2.5 py-1.5 text-white focus:border-indigo-500 focus:outline-none"
                  >
                    {daysOfWeek.map((d) => (
                      <option key={d} value={d}>
                        {d}
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div>
                  <label className="block text-slate-300 font-medium mb-1">Upto Date</label>
                  <input
                    type="date"
                    value={toDate}
          ...
          <div className="glass-card p-5 border border-slate-800 rounded-2xl bg-slate-900/70 backdrop-blur-md shadow-xl space-y-4 flex flex-col h-full">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-white">Transfer Requests & Approvals</h2>
                <span className="text-[11px] px-2 py-0.5 rounded-full bg-indigo-900/60 text-indigo-300 font-mono border border-indigo-700/40">
              ...
              <table className="w-full text-xs text-left">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-400 bg-slate-950/40">
                    ...
                    <th className="py-2.5 px-3 font-medium">Transfer Shift</th>
                    <th className="py-2.5 px-3 font-medium">Effective Period</th>
                  ...
                  {filteredTransfers.length === 0 ? (
                    ...
                    filteredTransfers.map((req) => (
                      <tr key={req.id} className="hover:bg-slate-800/30">
                        <td className="py-2.5 px-3 text-indigo-400 font-semibold">{req.id}</td>
                        <td className="py-2.5 px-3 font-sans">
                          <div className="font-medium text-slate-200">{req.empName}</div>
                        ...
                        <td className="py-2.5 px-3 font-sans">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="px-1.5 py-0.5 rounded bg-indigo-950 text-indigo-300 border border-indigo-800/50 font-semibold text-[10px]">
                              {req.requestedWeekoff}
                            </span>
                          </div>
                        </td>
                        <td className="py-2.5 px-3 text-[11px] text-slate-300">
                          {req.fromDate} <span className="text-slate-500">to</span> {req.toDate}
                        </td>
