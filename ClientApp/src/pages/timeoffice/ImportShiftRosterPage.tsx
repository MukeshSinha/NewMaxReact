    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">Import Shift Roster</h1>
      ...
      <div className="p-6 border-2 border-dashed border-slate-300 hover:border-indigo-500 rounded-2xl bg-white shadow-sm transition-all text-center space-y-4">
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-800">Select Excel File (.xlsx / .xls)</h3>
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <label
            htmlFor="fileUpload"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold cursor-pointer shadow-md shadow-indigo-600/20 transition-all"
          >
        ...
        {fileName && (
          <div className="inline-flex items-center gap-3 px-4 py-2 bg-indigo-50 border border-indigo-200 rounded-xl text-xs text-indigo-700">
            <span>
              Uploaded File: <strong className="text-slate-900">{fileName}</strong> ({rows.length} rows parsed)
            </span>
            <button
              type="button"
              onClick={handleClear}
        ...
        <div className="p-5 border border-slate-200 rounded-2xl bg-white shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-slate-800">Excel Sheet Preview (Dynamic Rows)</h2>
            </div>
          ...
          <div className="overflow-x-auto max-h-96 overflow-y-auto custom-scrollbar border border-slate-200 rounded-xl">
            <table className="w-full text-xs text-left" id="excelTable">
              <thead>
                <tr className="border-b border-slate-200 text-slate-600 bg-slate-50 sticky top-0">
                  ...
                  {headers.map((h, i) => (
                    <th key={i} className="py-2.5 px-3 font-semibold text-slate-800">
                      {h || `Col_${i + 1}`}
                    </th>
                  ))}
                </tr>
          ...
          <div className="pt-2 flex justify-end">
            <button
              ...
              disabled={importing}
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs shadow-lg shadow-emerald-600/20 transition-all flex items-center gap-2 disabled:opacity-60"
            >
