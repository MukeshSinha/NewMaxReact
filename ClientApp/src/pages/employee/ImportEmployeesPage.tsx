...
export const ImportEmployeesPage: React.FC = () => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (json && json.length > 0) {
          ...
          setCurrentPage(1);
          toast.success(`Loaded ${parsedRows.length} employee rows from ${file.name}`);
        } else {
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/Master/ImportEmpGeneralInfo', {
        method: 'POST',
        body: formData,
      });
      ...
      if (res.ok) {
        ...
        toast.error('Error occurred while uploading Excel file.');
      }
    } catch {
  ...
  return (
    <div className="w-full bg-[#f4f2f7] min-h-screen text-slate-800 p-6 font-sans">
      <div className="max-w-[100%] mx-auto space-y-6">
        {/* Page Header matching exact legacy screenshot icon & title */}
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-slate-800 tracking-tight">Employees</h2>
        </div>

        {/* File Upload Box */}
        <div className="bg-white p-4 rounded-md shadow-sm border border-slate-200 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-2">Select Employee Master Excel File (.xlsx / .xls)</label>
            <input
              type="file"
              accept=".xlsx, .xls"
          ...
          <div>
            <button
              ...
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-xs rounded shadow transition-colors flex items-center gap-2"
            >
              {isUploading ? (
                <>Uploading...</>
              ) : (
