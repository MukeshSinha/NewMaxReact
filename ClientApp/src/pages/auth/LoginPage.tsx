...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass-card p-8 relative z-10 space-y-6 border border-slate-200 shadow-xl bg-white">
        {/* Brand Header */}
        <div className="text-center space-y-2">
          <img src="/maxpay-logo.svg" alt="MaxPay Logo" className="w-16 h-16 mx-auto rounded-2xl shadow-md shadow-indigo-600/20 object-contain hover:scale-105 transition-transform duration-300" />
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-wide">MaxPay App</h1>
          <p className="text-xs text-slate-500">Contractor & Workforce Management Suite</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-300 rounded-xl pl-9 pr-4 py-2.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                required
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/20 transition-all tracking-wide disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isLoading ? (
              <>
